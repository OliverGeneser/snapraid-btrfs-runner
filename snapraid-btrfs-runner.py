...
def send_signal_notification(success, log):
    """
    ...
    send_command = [
        ...
        "-m",
        (
            f"✅ SnapRAID job completed successfully."
            if success
            else f"❌ Error during SnapRAID job: ``` {log} ```"
        ),
    ]
...
def snapraid_btrfs_command(
    command, *, snapraid_args={}, snapraid_btrfs_args={}, allow_statuscodes=[]
):
    """
    Run snapraid-btrfs command
    Raises subprocess.CalledProcessError if errorlevel != 0
    """
    snapraid_btrfs_arguments = [
        "--quiet",
        "--conf",
...
def finish(is_success):
    ...
    if "signal" in config and config["signal"]["enabled"]:
        if ("error", "success")[is_success] in config["signal"]["sendon"]:
            try:
                send_signal_notification(is_success, email_log.getvalue())
            except Exception:
                logging.exception("Failed to send Signal notification")
...
def load_config(args):
    ...
    config["snapraid-btrfs"]["pool"] = (
        config["snapraid-btrfs"]["pool"].lower() == "true"
    )
    config["snapraid-btrfs"]["cleanup"] = (
        config["snapraid-btrfs"]["cleanup"].lower() == "true"
    )

    if "signal" in config and config["signal"]["enabled"]:
        signal_chatid = config["signal"]["chatid"]
...
def setup_logger():
    ...
    if config["email"]["sendon"] or config["signal"]["sendon"]:
        global email_log
        email_log = StringIO()
...
def run():
    ...
    if shutil.which(config["snapraid-btrfs"]["executable"]) is None:
        logging.error(
            "Snapraid-btrfs executable does not exist at "
            + config["snapraid-btrfs"]["executable"]
        )
        finish(False)
    if shutil.which(config["snapper"]["executable"]) is None:
        logging.error(
            "Snapper executable does not exist at " + config["snapper"]["executable"]
        )
        finish(False)
    ...
    if len(config["snapraid-btrfs"]["snapper-configs"]) > 0:
        snapraid_btrfs_args_extend["snapper-configs"] = config["snapraid-btrfs"][
            "snapper-configs"
        ]
    if len(config["snapraid-btrfs"]["snapper-configs-file"]) > 0:
        snapraid_btrfs_args_extend["snapper-configs-file"] = config["snapraid-btrfs"][
    ...
    diff_out = snapraid_btrfs_command(
        "diff", snapraid_btrfs_args=snapraid_btrfs_args_extend, allow_statuscodes=[2]
    )
    ...
    if (
        ...
        try:
            snapraid_btrfs_command(
                "sync", snapraid_btrfs_args=snapraid_btrfs_args_extend
            )
    ...
    if config["snapraid-btrfs"]["pool"]:
        ...
        if len(config["snapraid-btrfs"]["pool-dir"]) > 0:
            snapraid_btrfs_args_extend["pool-dir"] = config["snapraid-btrfs"][
                "pool-dir"
            ]
        try:
            snapraid_btrfs_command(
                "pool", snapraid_btrfs_args=snapraid_btrfs_args_extend
            )
        except subprocess.CalledProcessError as e:
            logging.error(e)
            finish(False)
    ...
    if config["snapraid-btrfs"]["cleanup"]:
        ...
        try:
            snapraid_btrfs_command(
                "cleanup", snapraid_btrfs_args=snapraid_btrfs_args_extend
            )
        except subprocess.CalledProcessError as e:
            logging.error(e)
            finish(False)
    ...
    if config["scrub"]["enabled"]:
        ...
        try:
            snapraid_btrfs_command(
                "scrub",
                snapraid_args=snapraid_args_extend,
